...
# SELECT mst_id AS sensor, date::date AS date, SUM(flow_avg / 60) AS intensity
...
# def getHourlySumGroupFromIdPattern(pattern, start, end):
# "WITH pts AS \
...
#         ST_X(geom) lon, ST_Y(geom) lat, b.speed_avg, \
#         FROM pts AS a \
#             INNER JOIN ndw.trafficspeed AS b \
#             ON a.mst_id = b.location \
#             WHERE b.date >= '" + start + "' \
...
# WITH pts AS
# (SELECT * FROM ndw.mst_points_latest WHERE mst_id LIKE 'RWS01_MONIBAS_0021hrl%')
# SELECT a.location AS sensor, b.date::date AS date, SUM(b.flow_avg / 60) AS intensity, AVG(b.speed_avg), 
# ST_X(a.geom) AS lon, ST_Y(a.geom) AS lat
# FROM pts AS a
# INNER JOIN ndw.trafficspeed AS b
# ON a.mst_id = b.location
# WHERE b.date >= '06-01-2019'
# AND b.date <= '08-31-2019'
# GROUP BY a.location, date::date
# ORDER by a.location, date::date
